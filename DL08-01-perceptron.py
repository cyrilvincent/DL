...
diffFn = lambda x, y : abs(x - y)

class Perceptron:

    def __init__(self, id:str, weigths:List[float] = None, activationFn = reluFn, backPropagationFn = constantFn):
        self.id = id
        self.weigths:List[float] = weigths
        self.activationFn = activationFn
        self.backPropagationFn = backPropagationFn
    ...
    def backPropagation(self, loss:float, target:float, learningrate:float)->List[float]:
        ...
        for i in range(len(self.weigths)):
            ...
            delta = gradient * self.backPropagationFn(target) * learningrate
            self.weigths[i] -= delta
        return gradients

    def clone(self):
        p = Perceptron(self.id, self.weigths, self.activationFn, self.backPropagationFn)
...
if __name__ == '__main__':
    ...
    p0 = Perceptron("H1", [0.15,0.2,-0.35], sigmoidFn)
    p0.propagation([0.05,0.1])
    print(p0)

    #Layer
    #https://mattmazur.com/2015/03/17/a-step-by-step-backPropagation-example/
    hl = Layer("HL")
    hl.perceptrons.append(p0)
    hl.perceptrons.append(Perceptron("H2", [0.25,0.3], sigmoidFn, 0.35))
    print(hl)
    ...
    ol = Layer("OL")
    ol.perceptrons.append(Perceptron("O1",[0.4,0.45],sigmoidFn, dsigmoidFn))
    ol.perceptrons.append(Perceptron("O2", [0.5, 0.55], sigmoidFn, dsigmoidFn))
    network.layers.append(ol)
    print(network)
    network.propagation([0.05,0.1])
    print(network.outputs())
    print(network.computeLoss([0.01,0.99]))

    network.train([[0.05,0.1]],[[0.01,0.99]],1000)
    print(network.outputs())
